...
#### オプション構成（Phase 5: pH自動調整）

以下はPhase 5で追加するオプション部品。Phase 1-4はpHセンサー無しで完結する。
pHセンサーが無くても水耕栽培は十分可能。液肥の希釈倍率を守り、定期的に養液を交換すればpHは安定する。

| カテゴリ | 部品 | 型番 | 接続 | 概算価格 |
|---------|------|------|------|---------|
| pH | pHセンサー | PH-4502C or DFRobot SEN0161-V2 | アナログ→ADC→I2C | 1,000-2,000円 |
| ADC | アナログ-デジタル変換 | ADS1115 (16bit) | I2C | 500円 |
| ペリスタルティック | pH調整液投与 | DC12Vペリスタルティック x2 | L293Dモータードライバー | 3,000円x2 |
| モータードライバー | ポンプ制御 | L293D | GPIO | 300円 |
...
### Phase 5: pH自動調整（オプション）

Phase 5はオプションであり、Phase 1-4完了後に実施可能。pHセンサーが無くても水耕栽培は十分可能。

- [ ] pHセンサー（PH-4502C）+ ADS1115 ADC追加
- [ ] ペリスタルティックポンプでpH自動調整
- [ ] pH自動調整ロジック実装
- [ ] EC値に基づく栄養素自動投与（さらなる拡張）
- [ ] アラート通知（LINE or Discord）
...
| 品目 | 数量 | 単価 | 小計 |
|------|------|------|------|
| pHセンサー（PH-4502C） | 1個 | 1,000円 | 1,000円 |
...
| 構成 | 金額 |
|------|------|
| 必須構成（Phase 1-4） | 約19,300円 |
| オプション（Phase 5） | 約7,800円 |
| **全部入り** | **約27,100円** |
