...
| DCジャック→ネジ端子変換アダプタ | 5.5x2.1mm | 約200円 | Amazon | ACアダプタの丸型プラグを線に変換 |
| より線（撚り線） | AWG22 赤・黒 各1m | 約300円 | Amazon | リレー〜ポンプ間の配線 |
| **合計** | | **約4,000円** | | |
...
#### ACアダプタの出力端子がDCバレルジャック（丸型プラグ）の場合

一般的なAC-DC 12V電源は出力がDCバレルジャック（丸い端子）になっている。
このままではリレーに接続できないため、**DCジャック→ネジ端子変換アダプタ**を使って+/-の線を取り出す。

```
┌─────────────────┐     ┌─────────────────────────┐
│ AC-DC 12V電源   │     │ DCジャック→ネジ端子変換   │
│                 │     │                         │
│  出力: 丸型プラグ├────→│ ジャック ──→ (+)ネジ端子  │──→ 赤線でリレーCOM1へ
│  (5.5x2.1mm)   │     │            (-)ネジ端子  │──→ 黒線でポンプ(-)へ
│                 │     │                         │
└─────────────────┘     └─────────────────────────┘
```

- アダプタの極性マーク（⊕⊖）を確認。一般的にセンター（中心）がプラス(+)、外側がマイナス(-)
- Amazon等で「DCジャック ネジ端子」で検索（100〜300円程度）

#### 配線材料

| 材料 | 仕様 | 選定理由 |
|------|------|---------|
| **より線（撚り線）** | AWG22（0.3mm²）赤・黒 | 柔軟で取り回しやすい。単線は硬く折れやすいためNG |
| **フェルール端子**（推奨） | 0.34mm²用（ターコイズ色） | より線の先端をネジ端子に確実に接続するための圧着端子 |

- フェルール端子は「フェルール端子 圧着工具 セット」で検索（1,000〜1,500円。複数サイズ入りで今後も使える）
- フェルール端子が無い場合は、より線の先端をしっかりねじってネジ端子に挿せばDC12V程度なら使用可
- **はんだで先端を固めるのはNG**（ネジ端子内ではんだが潰れて接触不良・緩みの原因になる）
...
※ 「+12V out」「GND out」は DCジャック変換アダプタのネジ端子から取り出す
※ 配線にはAWG22より線（赤・黒）を使用し、端末にフェルール端子を圧着
```
